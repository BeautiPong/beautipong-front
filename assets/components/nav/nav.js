...
document.addEventListener('DOMContentLoaded', () => {
  ...
  logoutBtn.addEventListener('click', () =>
    showModal('정말 로그아웃하시겠습니까?', '확인', 'exit')
  );
  ...
  confirmBtn.onclick = async function () {
    try {
      ...
      const response = await fetch(
        `https://${SERVER_IP}/api/user/account/logout/`,
        {
          method: 'POST',
          headers: {
...
export function connectNotificationWebSocket(accessToken) {
  ...
  if (
    ...
    notificationWebSocket.readyState === WebSocket.OPEN
  ) {
  ...
  notificationWebSocket = new WebSocket(
    `wss://${SERVER_IP}/ws/user/?token=${accessToken}`
  );
  ...
  notificationWebSocket.onmessage = (event) => {
    ...
    console.log('nav에서 서버로부터 받은 data:', data);
    if (data.type === 'invite_game') {
      ...
      if (waitGamePage === null) waitGamePage = new WaitGamePage();
