...
export default class offlineGamePage {
  ...
  async afterRender() {
    ...
    // 상단 테두리
    const topBorderGeometry = new THREE.PlaneGeometry(
      tableWidth,
      borderThickness
    );
    ...
    // 좌측 테두리
    const leftBorderGeometry = new THREE.PlaneGeometry(
      borderThickness,
      tableLength
    );
    const leftBorderMaterial = new THREE.MeshPhongMaterial({
      color: borderColor,
    });
    ...
    socket.onmessage = function (event) {
      ...
      if (data.type === 'game_loop') {
        ...
        document.getElementById(
          'player1_nickname'
        ).textContent = `${data.state.player1}`;
        document.getElementById(
          'player2_nickname'
        ).textContent = `${data.state.player2}`;
        document.getElementById(
          'player1_score'
        ).textContent = `${data.state.player1_score}`;
        document.getElementById(
          'player2_score'
        ).textContent = `${data.state.player2_score}`;
    ...
    canvas.addEventListener('keydown', (event) => {
      ...
      if (socket == null) console.log('socket is null: ', socket);
