...
export default class OnlineGamePage {
  ...
  initTable(gameinfo) {
    ...
    // 앞쪽 테두리
    const frontBorderGeometry = new THREE.BoxGeometry(
    ...
    frontBorder.position.set(
      0,
      -(tableHeigth / 2),
      -(tableDepth / 2) - borderThickness
    ); // 앞쪽
    this.scene.add(frontBorder);

    // 뒤쪽 테두리
    const backBorderGeometry = new THREE.BoxGeometry(
      tableWidth + borderThickness * 2,
      tableHeigth,
      borderThickness
    );
  ...
  initPaddles(gameinfo) {
    const { paddleWidth, paddleHeight, paddleDepth, tableHeigth, tableDepth } =
      gameinfo;
  ...
  updateGameState(data, gameinfo) {
    ...
    if (this.playerRole === 'player1') {
      this.paddle.position.set(
        data.paddle_positions.player1,
        tableHeigth / 2,
        -(tableDepth / 2) + paddleDepth / 2
      );
      this.opponentPaddle.position.set(
        data.paddle_positions.player2,
        tableHeigth / 2,
        tableDepth / 2 - paddleDepth / 2
      );
    } else if (this.playerRole === 'player2') {
