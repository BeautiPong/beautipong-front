...
export default class WaitGamePage {
  ...
  updatePlayerInfo(data) {
    ...
    playerScore.textContent = `${data.match_cnt}전 ${data.win_cnt}승 ${
      data.match_cnt - data.win_cnt
    }패`;
  }
  ...
  async startMatch(friendNickname = null, host) {
    if (!friendNickname) this.showLoader(); // 로더를 표시하고 버튼을 숨김
  ...
  async fetchOpponentInfo(opponentNickname) {
    try {
      ...
      const response = await fetch(
        `http://${SERVER_IP}:8000/api/user/info/${opponentNickname}/`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  updateOpponentInfo(data) {
    ...
    opponentScore.textContent = `${data.match_cnt}전 ${data.win_cnt}승 ${
      data.match_cnt - data.win_cnt
    }패`;
  }
  ...
  async navigateToGamePage(roomName) {
    ...
    try {
      const accessToken = localStorage.getItem('access_token');
      const response = await fetch(
        `https://${SERVER_IP}/api/game/online/${roomName}/`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  handleWebSocketMessage(e) {
    ...
    if (data.type === 'game_start' && data.room_name) {
      ...
      const opponentNickname =
        nicknames[1] === myNickname ? nicknames[2] : nicknames[1];
    ...
    } else console.error('room_name is undefined');
  }
